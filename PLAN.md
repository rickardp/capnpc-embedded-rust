...
4. ~~Windows support.~~ **Done** — the runner preopens the cwd's drive root and
   translates paths to POSIX (`C:\a\b` -> `/a/b`) via `Path::components`. Verified
   by cross-target typecheck + the blocking `windows-latest` CI job. (Single-drive
   inputs only.)
5. Try to **upstream the ~9-file wasi patch set** to capnproto — it's small and
